...
***1. 数据传输的特点是：***
   ...
   3. 通过CHAN；1300 和CHAN；2400 这个命令来切换获取数据格式。温度抽取就是不停的轮询数据。采样时间是3s（一爆之后也是3S）。通讯方式如下：
...
对于HB M2S/M6S系列的主板来说，用的主控芯片是STM32的芯片，从性能上完去不需要用TC4指令传输指令的方式，厂商可以更新到Modbus-RTU方式传输的，效果更好，更稳定，物料成本基本一样。我也查了artisan，最早是2013年开始就支持modbus了。当然，现在也有用websocket的（比如狙击手，用的就是ESP32主控）

***2. 改造目标：***
   1. 无线连接方式传输数据到artisan
   2. 100%无损化版本：只能温度无线化读数（websocket），火力和风门数据自己手动标定。适合小白，在网上买了零件，对着教程，改一下接线，就可以实现功能。要返厂保修什么的，把附加的改装套件拆了就跟原装出厂的一样。
   3. 90%无损化版本：主控板上要进行部份改造。温度+火力无线化读数（websocket/Modbus），火力可以用artisan+编码器手动控制（那就是说可以自动烘了哦。小白肯定搞不定的，得机修佬搞的，要用到烙铁的。主控上拆下的零件可以原样焊接回去，功能完全不变更。所以叫做90%无损改造
   4. 完全替换主控板（实验板已经有的。程序也7788了。没时间测试。）
