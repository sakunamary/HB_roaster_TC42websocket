...
2.改造功目标：
   ...
   2. 100%无损化改造：只能温度无线化读数（websocket），火力和风门数据自己手动标定。适合小白，在网上买了零件，对着教程，改一下接线，就可以实现功能。要返厂保修什么的，把附加的改装套件拆了就跟原装出厂的一样。
   3. 90%无损化改造：主控板上要进行部份改造。温度+火力无线化读数（websocket/Modbus），火力可以用artisan+编码器手动控制（那就是说可以自动烘了哦。小白肯定搞不定的，得机修佬搞的，要用到烙铁的。主控上拆下的零件可以原样焊接回去，功能完全不变更。所以叫做90%无损改造
